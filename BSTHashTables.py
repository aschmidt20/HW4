...
bst_delete_elapsed = []  # the time it took for the binary tree to handle deletion of n elements
ht_insert_elapsed = []  # the time it took for the hash table to handle insertion of n elements
ht_delete_elapsed = []  # the time it took for the hash table to handle deletion of n elements
...
for n in n_vals:
    ...
    """
    ht_to_delete = dict((random_keys[i], random_vals[i]) for i in range(n))
    start = time.clock()
    for a in range(iters):
        hash_tab = ht_to_delete.copy()
        for i in range(n):
            del hash_tab[random_keys[i]]
    end = time.clock()
    ht_delete_elapsed.append((end - start)/iters)
    """

plt.semilogx(n_vals, ht_insert_elapsed, label = 'Hash Table insertion')
plt.semilogx(n_vals, bst_insert_elapsed, label = 'bst insertion')
# plt.semilogx(n_vals, ht_delete_elapsed, label = 'hash table deletion')
plt.legend()
plt.show()